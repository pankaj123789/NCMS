...
update tblSystemValue set Value = 'SDEULQJ6LH2VUJGA8PBBBXN0ZB2PIH' where ValueKey = 'XeroApiKey'
update tblSystemValue set Value = 'fb9fd734-d512-46f2-a4b4-8fa4563f33f2' where ValueKey = 'XeroSalesInvoiceBranding'
update tblSystemValue set Value = 'b44329e2-01db-4d3c-9420-cb14910fa69b' where ValueKey = 'XeroPaymentAccount'
...
IF NOT EXISTS (SELECT * FROM sys.sysusers WHERE name = N'$(admin_username)')
BEGIN
CREATE USER [$(admin_username)] FOR LOGIN [$(admin_username)] WITH DEFAULT_SCHEMA=[dbo]
END
GO
IF NOT EXISTS (SELECT * FROM sys.sysusers WHERE name = N'$(user_username)')
BEGIN
...
IF NOT EXISTS (SELECT * FROM sys.sysusers WHERE name = N'$(dev_username)')
BEGIN
...
IF NOT EXISTS (SELECT * FROM sys.sysusers WHERE name = N'$(dev_username2)')
BEGIN
