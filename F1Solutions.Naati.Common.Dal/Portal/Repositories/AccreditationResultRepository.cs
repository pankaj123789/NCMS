...
{
    public interface IAccreditationResultRepository : IRepository
    {
        bool IsRevalidationScheme(int naatiNo);
    ...
    public class AccreditationResultRepository : Repository, IAccreditationResultRepository
    {
        public AccreditationResultRepository(ICustomSessionManager sessionManager)
            : base(sessionManager)
        {
        }
        ...
        public bool IsRevalidationScheme(int naatiNo)
        {
            var entityId = GetEntityId(naatiNo);
            var queriedPerson = Session.Query<Person>().First(p => p.Entity.Id == entityId);
            bool isRevalidationScheme = queriedPerson != null && queriedPerson.RevalidationScheme == true;
                                                
            return isRevalidationScheme;
        }
    }
}
