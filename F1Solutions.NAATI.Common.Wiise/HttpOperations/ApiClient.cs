...
using F1Solutions.Naati.Common.Wiise.HttpOperations.Models;
...
namespace F1Solutions.Naati.Common.Wiise.HttpOperations
{
    ...
    internal class CustomJsonCodec : RestSharp.Serializers.ISerializer, RestSharp.Deserializers.IDeserializer
    {
        ...
        private readonly JsonSerializerSettings _serializerSettings = new JsonSerializerSettings
        {
            ...
            //changed to handle inserts where Wiise does not allow nulls
    ...
    public partial class ApiClient : ISynchronousClient, IAsynchronousClient
    {
        ...
        public ApiClient()
        {
            LoggingHelper.LogInfo ($"Value of BasePath {GlobalConfiguration.Instance.BasePath}");
        ...
        private ApiResponse<T> toApiResponse<T>(IRestResponse<T> response)
        {
            ...
            if (response.StatusCode != HttpStatusCode.OK && response.StatusCode != HttpStatusCode.Created && response.StatusCode != HttpStatusCode.NoContent)
            {
                ...
                if (apiResponse != null && apiResponse.error != null)
                    response.ErrorMessage = apiResponse.error.code + "|" + apiResponse.error.message;
                else
                    response.ErrorMessage = response.StatusCode + "|" + response.ErrorMessage;
            }
        ...
        private async Task<ApiResponse<T>> Exec<T>(RestRequest req, IReadableConfiguration configuration)
        {
            ...
            RestClient client = new RestClient(tempUrl);
            ...
            //REMOVE THIS WHEN WE GO OAUTH2
            //req.AddHeader("Authorization", "Basic V0lJU0VURVNUOnBXTVlybk1uT3BrKzNSU0lDQU5CeTA3Q2VFUU1BZHVtV0tFTzNDdjU1Mkk9");
            //wiise
            //req.AddHeader("Authorization", "Basic " + "TkFBVElUZWFtOmlxSG96blIrRzB0T2hMdjBOamgzQktoUStzcndjbXZsTWJQQnRvVTZOZ2s9");
            //our sandbox
            //req.AddHeader("Authorization", "Basic " + "QlJVQ0UuTUNMRU9EOnBGQlRqeXZuTWFubXA0T00yN0pCbG9WUjJDelROcVpDUVVEQ3JRZWpJUW89");

            InterceptRequest(req);

            IRestResponse<T> response = null;

            if (Convert.ToBoolean(ConfigurationManager.AppSettings["WiiseRandomErrorsEnabled"]))
            {
                response = await GenerateRandomResponse<T>(req, client);
            }
            else
            {
        ...
        private static async Task<IRestResponse<T>> GenerateRandomResponse<T>(RestRequest req, RestClient client)
        {
            IRestResponse<T> response = new MockApiResponse<T>();
            ...
            {
                response = new MockApiResponse<T>();
                response.StatusCode = 0;
                response.Content = "";
