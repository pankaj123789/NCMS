...
(function () {
    ...
    function recursiveUpdate(modelObj, viewModelObj, context, parentObj, noncontiguousObjectUpdateCount) {
        ...
        if (wasWrapped && (isNullOrUndefined(unwrapped) ^ isNullOrUndefined(modelObj))) {
            ...
            var newContext = { name: "[i]", parent: context.name + "[i]", full: context.full + "[i]", parentIsArray: true };

            viewModelObj.pushFromModel = function (item) {
                item = recrusiveFrom(item, null, newContext);
                viewModelObj.push(item);
            };
            viewModelObj.unshiftFromModel = function (item) {
                item = recrusiveFrom(item, null, newContext);
                viewModelObj.unshift(item);
            };
            viewModelObj.popToModel = function (item) {
                item = viewModelObj.pop();
                return recrusiveTo(item, newContext);
            };
            viewModelObj.shiftToModel = function (item) {
                item = viewModelObj.shift();
                return recrusiveTo(item, newContext);
            };
