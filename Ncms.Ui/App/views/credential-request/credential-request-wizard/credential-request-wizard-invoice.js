    ...
    function(applicationService) {
        ...
        function getInstance(params) {
            ...
            serverModel.InvoiceReference = ko.computed({
                read: function() {
                    var firstApplicant = vm.summary.Applicants()[0];
                    return serverModel.UserOfficeAbbreviation() +
                        ' - ' +
                        (firstApplicant || { ApplicationReference: ko.observable() }).ApplicationReference() +
                        (serverModel.ReferenceText() ? ' - ' + serverModel.ReferenceText() : '');
            ...
            vm.load = function() {
                ...
                var firstApplicant = vm.summary.Applicants()[0];
                var request = {
                    ApplicationId: firstApplicant.ApplicationId(),
                    ActionId: Number(vm.summary.Action().Id),
                    CredentialRequestId: firstApplicant.CredentialRequestId(),
                };
