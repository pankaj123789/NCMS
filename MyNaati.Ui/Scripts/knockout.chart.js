		...
		var scanForObservablesIn = function (model, subscribables) {
			for (var parameter in model) {
				...
				switch (typeOfData) {
					...
					default:
						{
							if (ko.isComputed(model[parameter]) || ko.isObservable(model[parameter])) { subscribables.push(model[parameter]); }
						}
						break;
				}
			}
		...
		ko.bindingHandlers.chart = {
			init: function (element, valueAccessor, allBindingsAccessor, viewModel) {
				...
				var refreshChart = function () {
					...
					//chartData.options = ko.toJS(chartBinding.options);
					//activeChart.update();
					//activeChart.resize();
				};
				...
				var subscribeToChanges = function () {
					...
					var dataSubscribables = [];
					//scanForObservablesIn(chartBinding.data, dataSubscribables);
					scanForObservablesIn(chartBinding, dataSubscribables);
