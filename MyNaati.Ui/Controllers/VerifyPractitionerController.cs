...
using MyNaati.Ui.ViewModels.VerifyPractitioner;
using System;
...
{
    public class VerifyPractitionerController : Controller
    {
        ...
        public VerifyPractitionerController(ICredentialQrCodeService credentialQrCodeService, ICredentialApplicationService credentialApplicationService, IApiPublicService apiPublicService)
        {
        ...
        public ActionResult Index(Guid? QrCode)
        {
            if(!QrCode.HasValue)
            {
                ...
                return View(new VerifyPractitionerModel() { Message = "Unable to find the matching practitioner. Please contact the practitioner or info@naati.com.au for more information" });
            }
            ...
            if (!serviceModel.AlowVerifyOnline)
            {
                ...
                return View(new VerifyPractitionerModel() { Message = "NAATI is unable to display this practitioner’s credentials.Please contact NAATI if you require verification." });
            }
            ...
            if (!getPhotoResult.Success)
            {
                ...
                return View(new VerifyPractitionerModel() { Message = getPhotoResult.Messages.First() });
            }

            var model = new VerifyPractitionerModel()
            {
                ...
                PastCertificationDetails = GetCertificationDetails(apiResult.PreviousCertifications),
