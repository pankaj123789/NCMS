	...
	function (credentialInProgress, credentialFuture) {
		...
		function getInstance(options) {
			var vm = {
				...
				availableCredentials: ko.observableArray(),
			...
			vm.credentialSelectorOptions = {
				...
				categoriesPromise: function () {
					...
					vm.credentialApplication.availableCredentials(req).then(function (data) {
						vm.availableCredentials(data);
						defer.resolve(data);
					});
				...
				levelsPromise: function () {
					return Q.when(selectedCategory().Children);
				},
				skillsPromise: function () {
					var req = $.extend(request(),
						{
							CredentialRequestPathTypeId: selectedLevel().CredentialRequestPathTypeId
						});
					return vm.credentialApplication.additionalSkills(req);
				},
			...
			function selectedCategory() {
				return ko.utils.arrayFirst(vm.availableCredentials(),
					function (c) {
						return c.Id === vm.credentialSelectorOptions.component.request.CategoryId();
					});
			}

			function selectedLevel() {
				var firstCredentialTypeId = vm.credentialSelectorOptions.component.request.CredentialTypes() ? vm.credentialSelectorOptions.component.request.CredentialTypes().toString().split(',')[0] : null;
				firstCredentialTypeId = parseInt(firstCredentialTypeId);
				return ko.utils.arrayFirst(selectedCategory().Children,
					function (c) {
						return c.Id === firstCredentialTypeId;
					});
			}
