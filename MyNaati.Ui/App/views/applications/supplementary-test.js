define(['services/requester'],
    function (requester) {
		...
		questionTypeControl[ENUMS.AnswerTypes.SupplementaryTestTasks] = 'views/applications/special-controls/supplementary-test-tasks';
        ...
        var vm = {
            ...
            title: ko.observable('Supplementary Test'),
		...
		vm.activate = function () {
            var pathArray = window.location.href.split('TestSittingId=');
			vm.testAttendanceId(parseInt(pathArray[1]));
            loadSettings();
        };
        ...
        function getNextQuestionFromSection(section) {
			...
			applications.post(request, 'supplementarytest/nextquestion').then(function (data) {
        ...
        function loadSettings() {
			applications.getFluid('supplementarytest/settings', { TestAttendanceId: vm.testAttendanceId() }).then(function (data) {
        ...
        function apply() {
			...
			applications.post(request, 'supplementarytest/apply').then(function (data) {
        ...
        function bindSpecialControl(question) {
            ...
            require([control], function (model) {
                var instance = model.getInstance({
					question: question,
					testAttendanceId: vm.testAttendanceId,
                    serviceName: 'supplementarytest',
                    naatiReference: "SUP-" + vm.testAttendanceId(),
                    naatiUnitType: "Supplementary Test Fee",
