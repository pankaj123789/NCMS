define(['services/requester'],
    function (requester) {
        ...
        function getInstance(options) {
            ...
            vm.fileOptions = {
                uploadUrl: window.baseUrl + 'logbook/createorupdateprofessionaldevelopmentactivityattachment',
                deleteUrl: window.baseUrl + 'logbook/deleteprofessionaldevelopmentactivityattachment',
            ...
            vm.load = function () {
                ...
                logbook.getFluid('getprofessionaldevelopmentactivityattachments', { ActivityId: vm.activityId() }).then(function (data) {
                    vm.attachments(data);
                });
            };

            vm.attachmentDownloadUrl = function (attachment) {
                return window.baseUrl + 'logbook/downloadactivityattachment/' + attachment.StoredFile.Id;
            };
            ...
            vm.removeAttachment = function (attachment) {
                mbox.remove({ title: 'Remove attachment', content: 'Are you sure that you want to remove this attachment?' }).then(function (answer) {
                    if (answer === 'yes') {
                        logbook.remove(attachment.Id, 'deleteprofessionaldevelopmentactivityattachment').then(function () {
                            vm.load();
