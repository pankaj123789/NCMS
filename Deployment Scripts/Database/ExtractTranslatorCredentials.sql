...
WHERE  cd.CredentialApplicationFieldId IN ( 3, 64 ) -- Products Claimed (transition) or Stamp (recertification)  
       AND cd.Value = 'True' -- Filter for requests where the products claimed is True.  
       AND c.CredentialTypeExternalName LIKE '%Translator%' -- Filter for Translator category  
